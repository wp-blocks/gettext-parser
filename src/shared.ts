...
export function parseNPluralFromHeadersSafely(
	...
	const { groups: { nplurals } = { nplurals: "" + fallback } } =
...
export function generateHeader(header: Record<string, string> = {}): string {
	...
	return (
		keys.map((key) => `${key}: ${(header[key] || "").trim()}`).join("\n") + "\n"
	);
}
 ...
 */
export function compareMsgid<T>(
	{ msgid: left }: { msgid: T },
	{ msgid: right }: { msgid: T },
): number {
	if (left < right) {
...
/**
 * Parses a Content-Type string to extract and update the charset
 */
export function updateContentTypeCharset(
	contentTypeStr: string,
	newCharset: string,
): string {
	const baseType = contentTypeStr.split(";")[0].trim();
	return `${baseType}; charset=${newCharset}`;
}

export function extractCharset(contentTypeStr: string): string | undefined {
	const match = contentTypeStr.match(/charset=([^;\s]+)/i);

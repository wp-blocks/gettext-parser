...
describe('PO Parser', () => {
  describe('headers', () => {
    test('should detect charset in header', async () => {
    ...
    test('should parse all known headers', async () => {
      const [po, json] = await Promise.all([
  ...
  describe('parsing errors', () => {
    ...
    test('should throw (stream with unescaped quote)', (t, done) => {
      ...
      stream.on('error', error => {
        assert.ok(error.message.match(invalidKeyError));
        done();
      });
    });
    ...
    describe('when validation is disabled', () => {
      ...
      test('should throw (unescaped quote)', async () => {
      ...
      test('should throw (double-escaped quote)', async () => {
        const po = await readFile(path.join(__dirname, 'fixtures/error-double-escaped-quote.po'));

        assert.throws(() => gettextParser.po.parse(po, options), invalidKeyError);
      });

      test('should not throw (an entry has too few plural forms)', async () => {
    ...
    describe('when validation is enabled', () => {
      ...
      test('should throw (an entry has too few plural forms)', async () => {
        const po = await readFile(path.join(__dirname, 'fixtures/validate-too-few-plural-forms.po'));

        assert.throws(() => gettextParser.po.parse(po, options), /Plural forms range error: Expected to find 3 forms but got 2 for entry "o1-2" in "" context\./);
      });

      test('should throw (an entry has too many plural forms)', async () => {
        const po = await readFile(path.join(__dirname, 'fixtures/validate-too-many-plural-forms.po'));

        assert.throws(() => gettextParser.po.parse(po, options), /Plural forms range error: Expected to find 2 forms but got 3 for entry "o1-2" in "" context\./);
      });

      test('should throw (an entry misses "msgid_plural")', async () => {
        const po = await readFile(path.join(__dirname, 'fixtures/validate-missing-msgid-plural.po'));

        assert.throws(() => gettextParser.po.parse(po, options), /Translation string range error: Extected 1 msgstr definitions associated with "o1-1" in "" context, found 2\./);
      });

      test('should throw (an entry misses single "msgstr")', async () => {
        const po = await readFile(path.join(__dirname, 'fixtures/validate-missing-msgstr.po'));

        assert.throws(() => gettextParser.po.parse(po, options), /Translation string range error: Extected 1 msgstr definitions associated with "o1" in "" context, found 0\./);
      });

      test('should throw (duplicate entries found in the same context)', async () => {
        const po = await readFile(path.join(__dirname, 'fixtures/validate-context-duplicate-entries.po'));

        assert.throws(() => gettextParser.po.parse(po, options), /Duplicate msgid error: entry "o1-1" in "c2" context has already been declared\./);
      });

      test('should throw (an entry with multiple "msgid_plural")', async () => {
        const po = await readFile(path.join(__dirname, 'fixtures/validate-redundant-msgid-plural.po'));

        assert.throws(() => gettextParser.po.parse(po, options), /Multiple msgid_plural error: entry "o1-1" in "" context has multiple msgid_plural declarations\./);
      });
    });
  });
});
